...
public abstract class Graph {
	...
	public List<Integer> degreeSequence() {
		// Create a temporary list to house all neighbor counts
		List<Integer> temp = new ArrayList<Integer>();
		
		// Get the sum of in and out neighbors for all vertices, and add to list
		for (int i=0; i<numVertices; i++) {
			int n = getNeighbors(i).size() + getInNeighbors(i).size();
			temp.add(n);
		}
		
		// Sort temp list in descending order
